...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BTCAParams"
    else
        PARAMS_DIR="$HOME/.btca-params"
    fi
fi
...
Failed to fetch the BTCA zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
BTCA - fetch-params.sh

This script will fetch the BTCA zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BTCA zkSNARK parameters. Note that it is
