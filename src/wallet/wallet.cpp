...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        ...
        else if (fMasterNode && tx->vout[i].nValue == 100 * COIN) {
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != 100 * COIN) {
        strError = "Invalid collateral tx value, must be 100 BTCA";
        ...
        const bool fIncludeLocked) const
{
    ...
    // Check for only 100 utxo
    if (nCoinType == ONLY_10000 && output.nValue != 100 * COIN) return res;
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of BTCA Node"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart BTCA Node to complete"));
            return nullptr;
        } else {
...
libzcash::SaplingPaymentAddress CWallet::GenerateNewSaplingZKey(std::string label) {
    if (!m_sspk_man->IsEnabled()) {
        throw std::runtime_error("Cannot generate anonymous addresses. Start with -upgradewallet in order to upgrade a non-HD wallet to HD and Sapling features");
    }
