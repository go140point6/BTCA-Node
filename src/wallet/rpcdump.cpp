...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        // Sapling keys
        // Let's see if the address is a valid BTCA spending key
        if (pwalletMain->HasSaplingSPKM()) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                if (addResult == KeyAlreadyExists) {
                    LogPrint(BCLog::SAPLING, "Skipping import of anonymous addr (key already present)\n");
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("bip38encrypt", "\"Aa7vXMBfxoVp1JcWGmd2F1cApdLm2dkLhb\" \"mypasphrase\"") +
            HelpExampleRpc("bip38encrypt", "\"Aa7vXMBfxoVp1JcWGmd2F1cApdLm2dkLhb\" \"mypasphrase\""));
...
UniValue exportsaplingviewingkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                "exportsaplingviewingkey \"anonymous_addr\"\n"
                "\nReveals the viewing key corresponding to 'anonymous_addr'.\n"
                "Then the importsaplingviewingkey can be used with this output\n"
                + HelpRequiringPassphrase() + "\n"

                "\nArguments:\n"
                "1. \"anonymous_addr\"   (string, required) The anonymous addr for the viewing key\n"
    ...
    if (!IsValidPaymentAddress(address)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid anonymous addr");
    }
    ...
    if (vk) {
        ...
        throw JSONRPCError(RPC_WALLET_ERROR, "Wallet does not hold private key or viewing key for this anonymous addr");
    }
}
...
UniValue exportsaplingkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                "exportsaplingkey \"anonymous_addr\"\n"
                "\nReveals the key corresponding to the 'anonymous_addr'.\n"
