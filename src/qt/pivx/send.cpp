    ...
    coinIcon(new QPushButton())
{
    ...
    ui->btnShieldCoins->setSubTitleClassAndText("text-subtitle", tr("Convert all transparent coins into anonymous coins"));
...
void SendWidget::refreshAmounts()
{
    ...
    QString type = isTransparent ? "transparent" : "anonymous";
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent BTCAs.
                             ...
                             const std::function<bool(QList<SendCoinsRecipient>&)>& func)
{
    ...
    if (isShieldedTx) {
        if (!walletModel->isSaplingEnforced()) {
            inform(tr("Cannot perform anonymous operations!"));
            return;
        }

        if (walletModel->isSaplingInMaintenance()) {
            inform(tr("Sapling Protocol temporarily in maintenance. Anonymous transactions disabled (SPORK 20)"));
            return;
        }
    }
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to anonymize BTCAs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent BTCAs to anonymize."));
    }
}
