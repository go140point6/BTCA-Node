...
PIVXGUI::PIVXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
    ...
    if (enableWallet) {
        ...
        centralWidget->setProperty("cssClass", "container-dashboard");
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("BTCA Node client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("BTCA Node"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("BTCA Node"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
