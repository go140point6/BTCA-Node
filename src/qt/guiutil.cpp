...
{
    return date.date().toString("MM/dd/yyyy").toUtf8().data() + QString(" ") + date.toString("hh:mm:ss");
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter BTCA address (e.g. %1)").arg("AUmz2jToqX2rZVpYMJnNBHQBRZcjgqASd4"));
...
bool parseBitcoinURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BTCA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTCA, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
