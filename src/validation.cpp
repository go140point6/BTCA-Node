                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
    ...
    if (!CheckTransaction(tx, consensus.NetworkUpgradeActive(chainHeight, Consensus::UPGRADE_ZC), true, state, false, fColdStakingActive, fSaplingActive))
...

    {
        ...
        if (!view.HaveShieldedRequirements(tx))
            return state.Invalid(error("AcceptToMemoryPool: anonymous requirements not met"),
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k BTCA
    ...
    if (nHeight > 6000000) return 0 * COIN;
    if (nHeight > nLast)   return 3  * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nChainSaplingValue) {
        if (*pindex->nChainSaplingValue < 0) {
            return state.DoS(100, error("%s: turnstile violation in Sapling anonymous value pool: val: %d", __func__, *pindex->nChainSaplingValue),
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (!fInitialBlockDownload) {
            if (fSaplingMaintenance && tx.IsShieldedTx())
                return state.DoS(100, error("%s : anonymous transactions are currently in maintenance mode", __func__));
        }
