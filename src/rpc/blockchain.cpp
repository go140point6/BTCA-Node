...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("anonymous_pool_value", ValuePoolDesc(blockindex->nChainSaplingValue, blockindex->nSaplingValue));
    if (blockindex->pprev)
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"nonce\" : n,           (numeric) The nonce\n"
            "  \"bits\" : \"1d00ffff\", (string) The bits\n"
            "  \"anonymous_pool_value\":   (object) Block anonymous pool value\n"
            "  {\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"anonymous_pool_value\": {  (object) Chain tip anonymous pool value\n"
    ...
    // Sapling shield pool value
    obj.pushKV("anonymous_pool_value", ValuePoolDesc(pChainTip->nChainSaplingValue, pChainTip->nSaplingValue));
