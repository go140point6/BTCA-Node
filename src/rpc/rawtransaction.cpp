...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"anonymous_addresses\"      (json array of string) the anonymous addresses involved in this transaction if possible (only for anonymous transactions and the tx owner/viewer)\n"
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the btca address, the value is the pivx amount\n"
            "      ,...\n"
            "    }\n"
